...
// T7 / T24 assert the NO-GRACE cutoff law. Since OD-5 (2026-09-25) the accepted policy carries
// graceHours 6, so these two pin graceHours 0 EXPLICITLY (same assertions, same instants) instead of
// relying on the default. The 6 h behaviour is proven in employee-grace-policy.test.mjs.
const POL0 = Object.assign({}, POL, { graceHours: 0 });
function clockedIn0() {
  const res = clockIn({ actor: emp('ans-a1'), shift: mkShift('ans-a1'), declaredStartAt: D(12), reasonCode: null, reasonNote: null, scope: SCOPE }, D(12), POL0);
  assert.ok(res.ok, 'clockedIn0 helper expected ok, got ' + res.code);
  return res.attendance;
}
t('T7 employee edit after deadline REJECT (graceHours 0 pinned)', () => {
  const a = clockedIn0();
  const r = employeeEdit({ actor: emp('ans-a1'), existing: a, patch: { note: 'x' }, scope: SCOPE }, NEXT(1), POL0);
  assert.equal(r.ok, false); assert.equal(r.code, 'EDIT_WINDOW_CLOSED');
});
t('T24 edit at EXACTLY the deadline instant REJECT (strictly-before; graceHours 0 pinned)', () => {
  const a = clockedIn0();
  const r = employeeEdit({ actor: emp('ans-a1'), existing: a, patch: { note: 'x' }, scope: SCOPE }, DEADLINE, POL0);
  assert.equal(r.ok, false); assert.equal(r.code, 'EDIT_WINDOW_CLOSED');
});
