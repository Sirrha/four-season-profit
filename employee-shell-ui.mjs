...
import { resolveEmployeeEntry, validateAdapters, denialMessage, DENIAL } from './employee-production-bridge.mjs';   // Employee Production Bridge: explicit production/preview entry + adapter seams
...
// ---- Employee Production Bridge: adapter seams. The shell reaches its ONE schedule truth, ONE
// attendance truth and the employee truth ONLY through ADAPTERS (contract: employee-production-
// bridge.mjs ADAPTER_CONTRACT). Preview binds the in-memory fixture stores below; production
...
function actorFromMembership(m) {
  return { uid: m.uid, accessRole: m.accessRole, ansattId: m.ansattId, accessEnabled: m.accessEnabled === true, tenantId: m.tenantId };
}

// ---- Shell ------------------------------------------------------------------
export function mountEmployeeShell(root, options) {
  if (!root) return;
  // ---- Employee Production Bridge: explicit entry mode. There is NO default mode: an absent or
  ...
  function resolveAssigneeIn(tenantId, ansattId) {
    if (MODE === 'preview' && (FOUR_SEASON_PEOPLE.some((x) => x.ansattId === ansattId) || VAKTPLAN_PEOPLE.some((x) => x.ansattId === ansattId))) return { status: 'FOUND', tenantId, ansattId };
    return employeeOf(employeeStore(), tenantId, ansattId) ? { status: 'FOUND', tenantId, ansattId } : { status: 'NOT_FOUND' };
  }
  // ---- Attendance persistence seam (S4 foundation). Preview: the Map seam is written directly, exactly as
  // before. Production: the adapter's commit() persists {attendance, event} (REV3 revision/server-time law) and
  // the mirror changes ONLY when the server confirms — never optimistic. A stale record is re-run ONCE against
  // the server copy through `rerun(existing)`; any failure is shown, and the surface re-renders from truth.
  let attendanceNotice = null;
  function attendanceErrorText(code) {
    switch (code) {
      case 'PERMISSION_DENIED': return 'Ingen tilgang – prøv å logge inn på nytt.';
      case 'UNAVAILABLE': return 'Ingen forbindelse – prøv igjen.';
      case 'STALE_REVISION': return 'Registreringen ble endret et annet sted – prøv igjen.';
  ...
  function goDenied(code) {
    clear(root);
    const idb = document.getElementById('emp-identity'); if (idb) { idb.hidden = true; idb.onclick = null; }
  ...
  function route(uid) {
    if (MODE !== 'preview') return goDenied(DENIAL.CHOOSER_PREVIEW_ONLY);   // fixture memberships are never production authority
  ...
  function setChrome(membership, view) {
    const person = personFor(membership);
  ...
  function openClockDialog(membership, shift, attId, kind) {
    ...
    dialogActions(card, kind === 'in' ? 'Bekreft innstempling' : 'Bekreft utstempling', () => {
      ...
      // production stale-record re-run (same operation, same inputs, server record injected)
  ...
  function openBreakDialog(membership, shift, attId, kind) {
    ...
    dialogActions(card, isStart ? 'Bekreft pausestart' : 'Bekreft pauseslutt', () => {
      ...
      const rerun = (ex) => (isStart ? startBreak({ actor, existing: ex, scope }, now, POLICY) : endBreak({ actor, existing: ex, scope }, now, POLICY));
      if (res.ok) { persistAttendance(attId, res, rerun); goToday(membership); return; }
      errBox.textContent = 'Kunne ikke registrere: ' + res.code;
    }, () => goToday(membership));
  }
  ...
  function openDeclareBreakDialog(membership, shift, attId) {
    ...
    dialogActions(card, 'Bekreft pausetid', () => {
      ...
      const rerun = (existing) => declareBreak({ actor, existing, declaredBreakMinutesTotal, reasonCode, reasonNote, scope }, now, POLICY);
      if (res.ok) { persistAttendance(attId, res, rerun); goToday(membership); return; }
      if (res.code === 'REASON_REQUIRED') {
  ...
  function goSchedule(membership) {
    ...
    const person = personFor(membership) || { ansattId: membership.ansattId, name: '' };
    ...
    for (const s of open) {
      ...
      const take = btn('Ta vakten', 'secondary', () => {
        const res = applyScheduleOperation({
          ...
          deps: {
            resolveAssignee: (a) => resolveAssigneeIn(membership.tenantId, a),   // preview: fixture people; production: the employee truth boundary
          },
        });
  ...
  // ---- Landing: preview lands on the default review identity (chooser via "Bytt ansatt");
...
// Structurally distinct doors. The preview page names preview explicitly; the production host
// (index.html after S2b-2/S3) supplies the shared resolved identity and the data adapters.
